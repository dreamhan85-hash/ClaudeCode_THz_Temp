#!/usr/bin/env bash
# paper-loop.sh — 논문 개선 루프 세션 회전 래퍼
#
# 매 반복마다 새 claude -p 세션을 생성하여 컨텍스트 초기화.
# paper_continuation.json의 상태를 기반으로 반복/종료 결정.
#
# Usage:
#   ./scripts/paper-loop.sh "results_dir=results/paper_260406 title=PE40_paper max_iterations=10 lang=ko"
...
STATE_FILE="${SESSION_DIR}/cache/paper_continuation.json"
...
while true; do
  ...
  ORCHESTRATOR_FILE=".claude/prompts/paper-orchestrator.md"
...
"
  ...
  case "$status" in
    submit_ready|stopped)
      log "Loop finished: status=$status"
      log "=== paper-loop.sh completed ==="
      exit 0
      ;;
    need_data)
      log "Need additional data — loop paused, waiting for /experiment"
      log "=== paper-loop.sh paused (need_data) ==="
      exit 0
      ;;
    improving)
